...
```{r enrolments plots, echo=FALSE, fig.width=7,fig.height=4.5, fig.cap= "Plots on enrollemnts numbers and completion rates over all runs"}
...
Two interesting areas to look at are gender and age. A question of whether gender or age range of participants affects engagement arises. To get an idea of how the gender differs across the hour runs the following bar plots are generated. These bar plots include data for all people that participated and not just the ones that completed the course: 
...
```
For both run 6 and 7 it seems that completion rate was the highest among people that are 26-35 years old. Moreover in all runs, except run 5, it seems that completion rate is also higher among people who are 36-45. People who are 18-25, don't seem to have very high completion rates. Moreover, what is very interesting is that in runs 6 and 7, among the people who completed the course and registered their age, all of them are below 45 years old. So this could be again an evidence that in general younger people between 26 and 45 years old seem to be the ones most interested in the course. 
